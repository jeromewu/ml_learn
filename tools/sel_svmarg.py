...
def worker_in(train_y, train_x, args, log_file_name, out_queue):
  args_str = ('-q -v %i -c %f -g %f') % (args['n_folds'], args['c'], args['g'])
  acc = svm_train(train_y, train_x, args_str)
  out_queue.put({'acc':acc, 'c':args['c'], 'g':args['g']})
  logger.info(('jobs finished: %i/%i, acc=%f, c=%f, g=%f') % (out_queue.qsize(), args['n_jobs'], acc, args['c'], args['g']))
  file_write(log_file_name, ('%f %f %f') % (acc, args['c'], args['g']))

def worker_out(train_y, train_x, test_y, test_x, args, log_file_name, out_queue):
...
def sel_svmarg_in(train_file_name=None, log_file_name=None, n_folds=0, c_start=-5, c_end=16, c_step=2, g_start=3, g_end=-16, g_step=-2):
  ...
  for c in c_list:
    for g in g_list:
      ...
      p = multiprocessing.Process(target=worker_in, args=(train_y, train_x, args, log_file_name, out_queue))
      jobs.append(p)
      p.start()
  for p in jobs:
    p.join()
...
def sel_svmarg_out(train_file_name=None, test_file_name=None, log_file_name=None, c_start=-5, c_end=16, c_step=2, g_start=3, g_end=-16, g_step=-2):
  ...
  for c in c_list:
    for g in g_list:
      ...
      p = multiprocessing.Process(target=worker_out, args=(train_y, train_x, test_y, test_x, args, log_file_name, out_queue))
      jobs.append(p)
      p.start()
  for p in jobs:
    p.join()
