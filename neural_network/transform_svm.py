...
def transform_svm(train_file_name, valid_file_name, test_file_name):
  train_label, train_feature_dict_array, train_n_dim = read_svm_from_file(train_file_name)
  valid_label, valid_feature_dict_array, valid_n_dim = read_svm_from_file(valid_file_name)
  test_label, test_feature_dict_array, test_n_dim = read_svm_from_file(test_file_name)

  n_dim = max([train_n_dim, valid_n_dim, test_n_dim])
  n_label = max(max(train_label), max(valid_label), max(test_label))+1

  train_feature = dict_to_numpy(train_feature_dict_array, n_dim)
  valid_feature = dict_to_numpy(valid_feature_dict_array, n_dim)
  test_feature = dict_to_numpy(test_feature_dict_array, n_dim)

  #slim svm feature
  del_arr = []
  train_feature_sum = train_feature.sum(axis=0)
  valid_feature_sum = valid_feature.sum(axis=0)
  test_feature_sum = test_feature.sum(axis=0)
  total_feature_sum = train_feature_sum + valid_feature_sum + test_feature_sum
  for i in xrange(0,len(total_feature_sum)):
    if total_feature_sum[i] == 0.0:
      del_arr.append(i)

  train_feature = np.delete(train_feature, del_arr, 1)
  valid_feature = np.delete(valid_feature, del_arr, 1)
  test_feature = np.delete(test_feature, del_arr, 1)
  n_dim -= len(del_arr)

  return (train_feature, train_label), (valid_feature, valid_label), (test_feature, test_label), n_dim, n_label

if __name__=='__main__':
  train_data, valid_data, test_data, n_dim, n_label = transform_svm(sys.argv[1], sys.argv[2], sys.argv[3])
  print n_dim, n_label
